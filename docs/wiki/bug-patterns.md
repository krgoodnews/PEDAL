...
| BP-009 | 알림 스크립트의 변수 치환 누락 및 강제 포커스 | `{feature}`, `{action}` 문자열이 실제 값으로 치환되지 않고 하드코딩된 채로 출력됨. 또한 알림 시 무조건 특정 앱을 `activate` 함 | `${1}`, `${2}` 인자 직접 처리 및 `osascript` 강제 활성화 코드 제거 | 셸 스크립트에서 알림을 보낼 때, 호출 측의 변수가 올바르게 전달되는지 인자($1, $2)를 통해 검증. 사용자 방해 금지를 위해 알림 시 창 활성화 지양 |
| BP-010 | 429 Lock Contention 에러 경로 테스트 누락 | 특정 에러 문자열("Failed to acquire lock")에 의존하는 로직을 작성했으나, 테스트에서 해당 문자열을 Mocking하지 않아 429 대신 500이 반환되는 것을 감지하지 못함 | test_server.py:70 | 429 반환을 검증하는 별도의 단위 테스트 추가 | 특정 에러 분기(특히 4xx 계열)가 있는 경우, 해당 분기 조건 문자열을 정확히 Mocking하여 HTTP 상태 코드가 의도대로 반환되는지 반드시 검증할 것 |
