...
## 크로스 리뷰 프로토콜 (Cross-Review)

PEDAL 사이클에서 문서를 작성하는 모든 단계는 작성한 에이전트와 **다른 AI 에이전트**에 의해 검토되어야 한다. (에코 챔버 방지)

**리뷰 횟수 및 종료 규칙:**
1. **최대 2회 제한**: 모든 문서 리뷰 프로세스는 최대 2회로 제한한다.
2. **2차 리뷰 조건**: 1차 리뷰 결과에 **Critical** 항목이 포함되고, 이를 수정했을 경우에만 2차 리뷰를 수행한다.
3. **루프 종료**: 2차 리뷰 후에는 결과(Critical 잔존 여부)와 상관없이 리뷰 루프를 종료한다. 
4. **에스컬레이션**: 2차 리뷰 후에도 Critical 항목이 해결되지 않는다면 사용자에게 보고하고 명시적 승인을 받거나 지시를 기다린다.

핵심 메커니즘:
- **크로스 리뷰**: 문서 작성 Agent와 다른 AI Agent가 리뷰 (최대 2회 제한 정책 적용)
