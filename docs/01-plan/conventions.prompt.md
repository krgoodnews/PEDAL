# Prompt Log: conventions

## [Plan] 2026-04-12T03:04:49Z

PEDAL 워크플로에 Convention을 넣고 싶어. 
현재 @01-plan/conventions.md 를 만들어놓았는데, 이런 내용을 Pedal 워크플로를 시작할 때 무조건 참조하게 하고, 또 위치와 네이밍이 적절한지 고민이 필요해.

(Content from @01-plan/conventions.md:
가능하다면 Engineering에 있는 항목들을 TDD(Test-Driven Development)로 구현할 것.
- Engineering 단계에서 정해놓은 기능들의 테스트 함수를 작성하고, 기능을 작성할 것.
- 개발 완료 후에 테스트 실행. 실패할 경우 원인 파악 후 해결
- 너무 많은 테스트 코드를 작성하지 말 것. PEDAL-Engineering 단계에 있는 항목과, Iterate 하게 될 때의 버그의 원인에 대해서만 테스트 코드를 작성해도 충분함.
- Analysis 단계에서 테스트를 수행할 것.)

## [Iterate] 2026-04-12T05:29:37Z

conventions 파일 이름을 대문자로 변경해줘. 
- as-is: conventions.md
- to-be: CONVENTIONS.md
