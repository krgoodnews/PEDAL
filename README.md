...
**PEDAL**은 전통적인 PDCA(Plan-Do-Check-Act) 사이클에서 영감을 받아, AI Agent 기반 소프트웨어 개발에 최적화된 워크플로입니다.

문서 기반의 체계적인 단계 진행과 **크로스 리뷰**, **지식 축적**, **반복 개선** 메커니즘을 통해, 체급이 낮은 AI Agent로도 일관되고 높은 품질의 결과물을 만들어냅니다.

---

## 왜 PEDAL인가?

AI Agent에게 "이거 만들어줘"라고 한 번에 맡기면, 결과물의 품질은 운에 달립니다. PEDAL은 이 문제를 구조로 해결합니다.

| 문제 | PEDAL의 해법 |
| --- | --- |
| Agent가 요구사항을 빠뜨림 | **Plan** 단계에서 범위와 요구사항을 명시적으로 문서화 |
| 설계 없이 바로 코딩하여 구조가 엉망 | **Engineering** 단계에서 아키텍처와 API를 먼저 설계 |
| 같은 Agent가 자기 결과물을 검증하면 편향 발생 | **크로스 리뷰**로 다른 회사의 AI가 비판적 검토 |
| 한 번 만들고 끝이라 품질 개선이 안 됨 | **Analyze → Iterate** 루프로 임계값 도달까지 자동 반복 |
| 다음 작업에서 이전 경험을 활용 못 함 | **Learn** 단계에서 Wiki로 지식을 축적하여 Agent가 재활용 |
...
대부분의 AI 개발 워크플로는 코드를 만들면 끝입니다. PEDAL은 다릅니다.
...
> PEDAL — 페달을 밟으면 앞으로 나아갑니다. 한 바퀴 돌 때마다 더 나은 코드가 됩니다.
